...
const (
	...
	userAgent      = "go-myob/" + libraryVersion
...
func (c *Client) SetCompanyFileID(companyFileID string) {
	c.companyFileID = companyFileID
}

func (c Client) CompanyFileID() string {
	return c.companyFileID
}
...
func (c *Client) GetEndpointURL(p string, pathParams PathParams) url.URL {
	...
	clientURL.Path = path.Join(clientURL.Path, c.companyFileID, parsed.Path)
...
func (c *Client) NewRequest(ctx context.Context, req Request) (*http.Request, error) {
	...
	r.Header.Add("x-myobapi-version", "v2")
	r.Header.Add("x-myobapi-key", c.ClientID())
